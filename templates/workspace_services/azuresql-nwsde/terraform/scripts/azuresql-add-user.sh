...
echo "Testing connection to SQL Server: $SERVER_FQDN"

while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do

  if SQLCMD_OUTPUT=$(sqlcmd --server "$SERVER_FQDN" --database-name master --use-aad --query "select 1;" --exit-on-error 2>&1); then
    echo -e "Attempt $((ATTEMPT+1)):  SQL connection made to server $SERVER_FQDN, continuing\n"
